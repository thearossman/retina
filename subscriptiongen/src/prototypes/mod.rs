...
pub use self::connection::{ConnectionData, ConnectionSubscription};
pub use self::http::{HttpSubscription, HttpTransactionData};
...
pub(crate) fn build_field(
    ...
    return match field_name {
        "tls" => TlsSubscription::delivered_field(),
        "http" => match is_connection {
        ...
        "connection" => ConnectionSubscription::delivered_field(),
        _ => {
            panic!("Unknown field")
        }
    };
...
pub(crate) fn build_condition(field_name: &str) -> Option<proc_macro2::TokenStream> {
    match field_name {
        "tls" => Some(TlsSubscription::condition()),
        "http" => Some(HttpSubscription::condition()),
        _ => None,
    }
}
