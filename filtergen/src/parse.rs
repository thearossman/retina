...
use syn::{parse::{Parse, ParseStream}, Item, ItemFn, LitStr, TypePath};
use quote::ToTokens;
        ...
        let returns = match &func.sig.output {
            ...
            syn::ReturnType::Type(_, ty) => match ty.as_ref() {
                syn::Type::Path(type_path) => {
                    if type_path.path.is_ident("bool") { FnReturn::Bool }
                    else if type_path.path.is_ident("FilterResult") { FnReturn::FilterResult }
                    else if type_path.path.is_ident("Self") ||
                            type_path.path.is_ident(&name)
                    {
                        FnReturn::Constructor(Constructor::Sel)
                    }
                    else if type_path.path.segments.len() == 1 && type_path.path.segments[0].ident == "Option" {
                        Self::parse_option(type_path, &name)
                    }
                    else { panic!("{}", ParserError::InvalidReturn(name.clone())); }
                },
    ...
    pub(crate) fn parse(&mut self, input: &Item, args: Option<String>) -> Result<()> {
        match &input {
            syn::Item::Fn(itemfn) => {
                ...
                match self {
                    ...
                    Self::Callback(func) => {
                        ...
                        let (fil, level) = InputKeys::callback(args, &spec.name)?;
                        func.filter = fil;
                        func.level = level;
                        func.func = spec;
                    },
                    ...
                    Self::Filter(func) => {
                        ...
                        if let Some(args) = args {
                            ...
                            if keys.first.is_some() {
                                bail!(ParserError::InvalidParam(keys.first.unwrap(), spec.name.clone()));
                            }
                        }
                        func.func = spec;
                    },
                    ...
                    Self::DatatypeFn(func) => {
                        if !matches!(spec.returns, FnReturn::None |
                                     FnReturn::Constructor(_)) {
                            bail!(ParserError::InvalidReturn(spec.name));
                        }
                        let (group_name, level) = InputKeys::grouped_fn(args, &spec.name)?;
                        func.group_name = group_name;
                        ...
                        assert!(func.level.iter().all(|l| l.is_streaming())
                                || func.level.iter().all(|l| !l.is_streaming()),
                                    "{}:{} mixed streaming and non-streaming states",
                                    func.group_name, func.func.name);
                        assert!(func.func.datatypes.len() == 1,
                                "{}:{} requested multiple datatypes",
                                func.group_name, func.func.name);
                        assert!(func.func.datatypes[0].contains("L4Pdu") ||
                        ...
                        match args {
                            Some(args) => {
                                ...
                                match keys.first {
                                    ...
                                    None => bail!(ParserError::MissingParam("name".into(), spec.name.clone())),
                                }
                                dt.level = match keys.levels.len() {
                                    0 => None,
                                    1 => Some(keys.levels.pop().unwrap()),
                                    _ => bail!(ParserError::InvalidParam("[levels > 1]".into(), spec.name.clone())),
                                };
                            },
                            None => bail!(ParserError::MissingParam("name".into(), spec.name.clone())),
                        }
            ...
            syn::Item::Struct(st) => {
                ...
                match self {
                    Self::Callback(_) => {
                        let (filter, mut level) = InputKeys::callback(args, &name)?;
                        let level = match level.len() {
                            0 => None,
                            1 => Some(level.pop().unwrap()),
                            _ => bail!(ParserError::InvalidParam("[levels > 1]".into(), name.clone())),
                        };
                    ...
                    Self::Filter(_) => {
                        *self = ParsedInput::FilterGroup(
                            FilterGroupSpec {
                                level: InputKeys::struct_def(args, &name)?,
                                name: name,
                            }
                        );
                    },
                    Self::FilterGroup(fil) => {
                        fil.level = InputKeys::struct_def(args, &name)?;
                        fil.name = name;
                    },
                    Self::Datatype(dt) => {
                        dt.level = InputKeys::struct_def(args, &name)?;
                        dt.name = name;
                    },
...
impl InputKeys {
    fn from_string(inp: String) -> Result<Self> {
        let parsed: Vec<(String, String)> = inp.clone().split(',').enumerate().map(|(i, pair)| {
            let pair = pair.trim();
        ...
        for (k, v) in parsed {
            ...
            if k.is_empty() {
                assert!(ret.first.is_none(),
                        "Multiple values without keys: {}", inp);
                ret.first = Some(v);
            }
            else if k.contains("filter") {
                assert!(ret.first.is_none(),
                        "Expect filter to be first or labeled: {}", inp);
                ret.first = Some(v);
            }
            else if k.contains("group") {
                assert!(ret.first.is_none(),
                        "Expect group to be first or labeled: {}", inp);
                ret.first = Some(v);
            }
                ...
                reassembled = bool::from_str(&v)
                    .expect(&format!("{}", ParserError::InvalidValue(k, v)));
            }
            else if k.contains("name") {
                assert!(ret.first.is_none(),
                        "Expect name to be first or labeled: {}", inp);
                ret.first = Some(v);
            }
            else {
    ...
    fn callback(args: Option<String>, name: &String) -> Result<(String, Vec<DataLevel>)> {
        let mut ret = (String::new(), Vec::new());
        match args {
            Some(args) => {
                let keys = InputKeys::from_string(args)?;
    ...
    fn struct_def(args: Option<String>, name: &String) -> Result<Option<DataLevel>> {
        let mut ret = None;
        if let Some(args) = args {
            let mut keys = InputKeys::from_string(args)?;
