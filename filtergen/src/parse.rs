...
use quote::quote;
use retina_core::filter::{ptree::PTree, Filter};
...
pub(crate) fn get_configs() -> Value {
    ...
    let f_in = std::fs::File::open(&filepath_in).expect(&format!(
        "Failed to read config filepath ({})",
        &filepath_in
    ));
    let data_in: Value = from_reader(f_in).expect("Failed to read subscription config");
...
fn get_collapsed_filter(filters: &Vec<String>) -> String {
    ...
    let collapsed_ptree = Filter::from_str(&collapsed_filter, false, 0)
        .expect(&format!(
            "Failed to generate collapsed filter: {}",
            &collapsed_filter
        ))
        .to_ptree(0);
...
pub(crate) fn get_callbacks_from_config(data_in: Value) -> proc_macro2::TokenStream {
    ...
    for (k, v) in iter {
        ...
        for idx in idxs {
            ...
            callback_names.push((quote! { Box::new(#callback_name), }, idx.as_i64().unwrap()));
        }
    }
    callback_names.sort_by(|a, b| a.1.cmp(&b.1));
