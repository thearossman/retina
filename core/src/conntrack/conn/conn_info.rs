    ...
    T: Trackable,
{
    ...
    pub(crate) fn new_packet(&mut self, pdu: &L4Pdu,
        ...
        if self.linfo.state == LayerState::Payload &&
           ...
           self.tracked.update_l4_payload(pdu)
        {
            self.exec_state_tx(StateTransition::L4InPayload, subscription);
        }
        // TODO pass OOO to L7 layer??
    }
    ...
    /// Invoked by transport layer (in order received for UDP, post-reassembly for TCP)
    /// to update data for encapsulated layers.
        ...
        registry: &ParserRegistry)
    {
        ...
        let tx_ = self.layers[0].process_packet(pdu, &mut self.tracked, registry);
        for tx in tx_ {
            self.exec_state_tx(tx, subscription);
            if self.layers[0].needs_process(tx) {
                self.layers[0].process_packet(pdu, &mut self.tracked, registry);
            }
        }
    }
