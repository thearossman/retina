...
pub(crate) trait TrackableLayer {
    /// Ingest the next packet, returning State transition(s) triggered.
    ...
    /// if process_packet needs to be called again.
...
impl TrackableLayer for Layer {
    ...
    fn process_packet<T: Trackable>(
        ...
        match self {
            Layer::L7(session) => session.process_packet(pdu, tracked, registry),
        }
    }
...
impl TrackableLayer for L7Session {
    ...
    fn needs_process(&self, tx: StateTransition) -> bool {
        // Disc -> Headers state transition: need to call process_packet
        // to handle the first header packet.
        tx == StateTransition::L7OnDisc
        // TODO handle L7EndHdrs - if payload starts in same pkt as hdrs end
    }
    ...
    fn process_packet<T: Trackable>(
        ...
        match self.linfo.state {
            ...
            LayerState::Payload => {
                if tracked.update_l7_payload(pdu) {
